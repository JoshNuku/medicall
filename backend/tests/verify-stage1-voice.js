...
async function runTests() {
  ...
  const confirmXml = await processReminderConfirm(callEvent.id, '1', baseUrl);
  ...
  await processReminderConfirm(callEvent2.id, '2', baseUrl);
  ...
  const repeatXml = await processReminderConfirm(callEvent.id, '9', baseUrl);
  ...
  const helpXml = await processReminderConfirm(callEvent.id, '0', baseUrl);
